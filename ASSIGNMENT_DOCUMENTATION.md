...
**Testing approach**: Verified that the synchronization objects were correctly initialized in the SharedResources clas
...
**How I solved it**: aced the unlock() method strictly inside the finally block to guarantee the lock is always released
...
**Justification**: 
Using ReentrantLock ensures Mutual Exclusion, allowing only one thread to update the counter at a time. The try-finally block guarantees that the lock is always released, preventing deadlocks and ensuring data integrity
...
I used a dedicated lock to ensure Mutual Exclusion during write operations to the list. This guarantees that each log entry is safely added and preserved, maintaining a reliable record of the simulation's events without crashing the program.</String>
...
It prevents multiple processes from executing their "quantum" simultaneously This enforces the Mutual Exclusion principle for the CPU resource, resulting in a realistic and synchronized Round Robin scheduling behavior where processes wait their turn in the queue
...
The implementation of Locks and Semaphores successfully synchronized the multithreaded environment These mechanisms prevented race conditions and ensured consistent reliable output for the Round Robin scheduling simulation, even under concurrent thread execution
...
**Repository URL**:[https://github.com/makopt/OS-Assignment3-Starter](https://github.com/makopt/OS-Assignment3-Starter) 
...
Successfully synchronizing a complex multithreaded environment where the final simulation statistics perfectly match the theoretical Round Robin calculations for my Student ID
